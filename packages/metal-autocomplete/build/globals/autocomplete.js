	...
	goog.require = function (name) {
		try {
			...
			if (parts[parts.length - 1] === 'incrementaldom') {
				...
				console.warn('The template with namespace "' + name + '" was required before being ' + 'declared via "goog.module". Make sure to import any soy components (or ' + 'templates) before importing your component\'s own soy file that uses ' + 'them, to avoid this error.');
			}
			throw error;
		}
	};
  ...
  */
   ...
   */
		...
		IncrementalDomRenderer.prototype.renderWithoutPatch = function renderWithoutPatch() {
			...
			IncrementalDomAop.startInterception(this.handleInterceptedOpenCall_.bind(this), this.handleInterceptedCloseCall_.bind(this));
			this.renderIncDom();
